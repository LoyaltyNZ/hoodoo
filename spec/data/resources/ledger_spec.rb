...
describe ApiTools::Data::Resources::Ledger do
  it 'should match schema expectations' do
    ...
    expect(schema.properties['reason']).to be_a(ApiTools::Presenters::Enum)
    expect(schema.properties['reason'].from).to eq(['calculation', 'manipulation'])
    expect(schema.properties['reason'].required).to eq(true)

    expect(schema.properties['debit']).to be_a(ApiTools::Presenters::Object)
    expect(schema.properties['debit'].required).to eq(false)

    expect(schema.properties['credit']).to be_a(ApiTools::Presenters::Object)
