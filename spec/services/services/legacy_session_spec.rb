...
describe Hoodoo::Services::LegacySession do
  ...
  describe '#self.load_session' do
    ...
    it 'should raise an error if memcache_url is nil' do
      expect {
        Hoodoo::Services::LegacySession.load_session(nil, '0123456789ABCDEF')
    ...
    it 'should raise an error if memcache_url is empty' do
      expect {
        Hoodoo::Services::LegacySession.load_session('', '0123456789ABCDEF')
      }.to raise_error "Hoodoo::Services::Middleware memcache server URL is nil or empty"
    end

    it 'should return nil if session_id is nil' do
      session = Hoodoo::Services::LegacySession.load_session('url', nil)
      expect(session).to be_nil
    end

    it 'should return nil if session_id is empty' do
      session = Hoodoo::Services::LegacySession.load_session('url', '')
      expect(session).to be_nil
    end

    it 'should return nil if session_id is less than 32 chars' do
      session = Hoodoo::Services::LegacySession.load_session('url', '0123456789ABCDE')
      expect(session).to be_nil
    end

    it 'should call connect_memcache and raise error if return is nil' do
      expect(Hoodoo::Services::LegacySession).to receive(:connect_memcache).with('url').and_return(nil)
      expect {
        Hoodoo::Services::LegacySession.load_session('url', '0123456789ABCDEF0123456789ABCDEF')
    ...
    it 'should call get on memcache with correct key and return nil if not found' do
      ...
      expect(Hoodoo::Services::LegacySession).to receive(:connect_memcache).and_return(mock_memcache)

      session = Hoodoo::Services::LegacySession.load_session('url', '0123456789ABCDEF0123456789ABCDEF')
      expect(session).to be_nil
    end
  ...
  describe '#connect_memcache' do
    it 'should call Dalli and return a new client' do
      ...
      expect(Hoodoo::Services::LegacySession.send(:connect_memcache, 'one')).to eq(mock_memcache)
    end
    ...
    it 'should return nil if Dalli::Client.new raises an error' do
      ...
      expect(Hoodoo::Services::LegacySession.send(:connect_memcache, 'one')).to be_nil
    end
  ...
  describe '#initialize' do
    it 'should load correct options' do
      s = Hoodoo::Services::LegacySession.new({
        :participant_id => "TESTPART1",
        :outlet_id => "TESTOUTLET1",
        :roles => "TESTROLE1,TESTROLE2",
      })
