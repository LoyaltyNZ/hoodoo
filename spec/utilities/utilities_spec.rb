...
describe Hoodoo::Utilities do
  ...
  describe Hoodoo::Utilities::CIHash do
    shared_examples 'CIHash' do
      it 'allows case insensitive String lookup' do
        expect( @hash[ 'ONE' ] ).to eq( 1 )
        expect( @hash[ 'TWO' ] ).to eq( 2 )
      end

      it 'allows case insensitive Symbol lookup' do
        expect( @hash[ :ONE ] ).to eq( 1 )
        expect( @hash[ :TWO ] ).to eq( 2 )
      end
    end
    ...
    context 'with ::[] construction' do
      ...
      it_behaves_like 'CIHash'
    end

    context 'with #[]= construction and a parameter' do
      before :each do
        @hash = described_class.new( 23 )
        @hash[ 'one' ] = 1
        @hash[ :two  ] = 2
      end
      ...
      it 'has the expected default value' do
        expect( @hash[ :three ] ).to eq( 23 )
      end
    end

    context 'with #[]= construction and a block' do
      before :each do
        @hash = described_class.new do | hash, key |
          hash[ key ] = "key #{ key }"
        end
      ...
      it 'has the expected default value' do
        expect( @hash[ :three ] ).to eq( "key THREE" )
      end
    end

    context 'with #[]= construction and no parameter or block' do
      before :each do
