...
describe ApiTools::Presenters::UUID do
  describe '#validate' do
    it 'should return no errors when valid UUID' do
      expect(ApiTools::Presenters::UUID.new('one').validate(ApiTools::UUID.generate()).errors).to eq([])
    end
    ...
    it 'should return correct error when data is not a UUID-like string (2)' do
      errors = ApiTools::Presenters::UUID.new('one').validate('01234567890123456789012345678901z')
    ...
    it 'should return correct error when data is not a UUID-like string (3)' do
      errors = ApiTools::Presenters::UUID.new('one').validate('3ba258c0-4ce3-0132-9862-28373700b71c')
    ...
    it 'should return correct error when data is not even a string (2)' do
      errors = ApiTools::Presenters::UUID.new('one').validate(ApiTools::UUID.generate().to_sym)
