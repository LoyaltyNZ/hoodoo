...
describe ApiTools::Presenters::Float do
  ...
  describe '#validate' do
    ...
    it 'should return correct error when data is not a float' do
      ...
      err = [  {:code=>"generic.invalid_float", :message=>"The field at `one` is an invalid float", :reference=>"one"}]
    ...
    it 'should return correct error with path' do
      ...
      expect(errors).to eq([
       {:code=>"generic.invalid_float", :message=>"The field at `ordinary.one` is an invalid float", :reference=>"ordinary.one"}
      ])
    end
  end
end