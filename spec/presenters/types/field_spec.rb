...
describe ApiTools::Presenters::Field do
  ...
  describe '#validate' do
    ...
    it 'should return correct error when required and data is nil' do
      ...
      errors = @inst.validate(nil)
      expect(errors).to eq([
        {:code=>"generic.required_field_missing", :message=>"The field at `one` is required", :reference=>"one"}
      ])
    end
    ...
    it 'should return correct error with path' do
      ...
      expect(errors).to eq([
       {:code=>"generic.required_field_missing", :message=>"The field at `ordinary.one` is required", :reference=>"ordinary.one"}
      ])
    end
  end
